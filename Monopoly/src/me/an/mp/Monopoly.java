...
public class Monopoly extends Canvas implements Runnable, MonoListener
{
	...
	private int turnTime = Defaults.TURN_TIME * 60, maxTurnTime = Defaults.TURN_TIME * 60;
	...
	private void tick()
	{
		...
		{
			if (getMostRecentMenu() instanceof BuyMenu)
				return;

			turnTime--;

			double timePercent = ((double) turnTime) / ((double) maxTurnTime);
			gameMenu.getTurnTimeBar().setPercent(timePercent);
	...
	public void onEvent(MonoEvent event)
	{
		...
		if (event instanceof DiceRollEvent)
		{
			...
			if (dice.isDoubles() && movePlayer == player)
				turnTime = maxTurnTime;

			if (movePlayer.inJail())
				movePlayer.setInJail(false);
	...
	private void cycleTurn()
	{
		...
		if (turnPlayer == player)
		{
			turnTime = maxTurnTime;
			dice.show(player);
			gameMenu.showTurnTimer(true);
		} else
		{
			gameMenu.showDoneButton(false);
			gameMenu.showTurnTimer(false);
		}
